/** ══════════════════════════════════════════════════════════════════
 *  ⚡ ALL-IN-ONE — الصق هذا الملف كاملاً في Code.gs واحد
 *  ────────────────────────────────────────────────────────────────
 *  خطوات التركيب (دقيقتان):
 *   1) افتح الجدول ← Extensions ← Apps Script
 *   2) إن وُجد كود قديم: احذفه كاملاً (بعد الاحتفاظ بنسخة عندك)
 *   3) الصق هذا الملف كاملاً مكان Code.gs ← احفظ (Ctrl+S)
 *   4) Project Settings ⚙️ ← Time zone: (GMT+03:00) Riyadh
 *   5) شغّل initializeSystem ← وافق على الصلاحيات
 *   6) Project Settings ⚙️ ← Script Properties ← أضف المفاتيح
 *      (القائمة الكاملة في automation/SETUP-AR.md)
 *   7) من الجدول: قائمة 🤖 الأتمتة ← 🔑 فحص المفاتيح
 *
 *  الملف يجمع: النظام الأساسي + جسر Life Pulse PUBLISH_QUEUE
 *  + استيراد الأفكار المعتمدة من CONTENT_LIBRARY
 * ══════════════════════════════════════════════════════════════════ */
...
function sendNotification(subject, message, type) {
  ...
  const body = message + '\n\nالوقت: ' + new Date() + '\nالجدول: ' + SpreadsheetApp.getActiveSpreadsheet().getUrl();
  if (email) {
...
/* ════════════ القسم الثاني: جسر Life Pulse PUBLISH_QUEUE ════════════ */
